...
public class BankTransactionController : ControllerBase
{
    ...
    public BankTransactionController(BankTransactionService bankTransactionService,MichuBankContext context,AccountService accountService)
    {
        _context = context;
    ...
    public async Task<IActionResult> CreateRetiro(BankTransactionDtoIn bankTranDto, int AccountId)
    {
       //obteniendo la info de los claims
        ...
        //validar la cuenta 
        var account = await _context.Accounts.FirstOrDefaultAsync(a => a.Id == AccountId && a.Client.Name == name);

        if (account == null)
            return BadRequest("Ups! esta no es tu cuenta");
    ...
    public async Task<IActionResult> DeleteCuentas(int AccountId)
    {
        ...
        if (account == null)
            return BadRequest("Ups!, lo siento no puedes efectuar este movimiento si no eres duenio de la cuenta");
        ...
        if(balance == 0) 
        {
            ...
            return BadRequest("Lo sentimos no puedes borrar tu cuenta!, debe de estar en $0.00");
